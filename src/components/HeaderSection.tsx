...
export const HeaderSection = () => {
  return <section className="relative pt-24 pb-8 bg-gradient-to-b from-background via-background to-card overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          duration: 0.6
            ...
            <img src={logo} alt="SM Racing Logo" className="h-24 w-24 md:h-32 md:w-32 rounded-full object-cover border-4 border-primary glow-primary" />
          ...
          delay: 0.2
            ...
            <div className="relative">
              <div className="flex gap-4 animate-photo-scroll">
                {photos.map((photo, index) => <div key={index} className="flex-shrink-0 w-32 h-20 md:w-48 md:h-28 rounded-lg overflow-hidden border border-border">
                    <img src={photo} alt={`Racing moment ${index + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" />
                  </div>)}
              </div>
